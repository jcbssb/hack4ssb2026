...
-- | Trial 5: 20Byggesak rebuilt from the filled-in PDF (research/20byggesak-pdf-tekst.txt),
-- see plans/byggesak-trial5-plan.md.
--
...
trial5ByggesakDialogue :: Dialogue
trial5ByggesakDialogue = Dialogue
...
bolker =
  [ bolk "bolk_a" "A. Opplysninger om skjema og kontaktinformasjon"
      ...
      Nothing
      [ questionsOnly
          [ field "t5_kommunenummer" "Kommunenummer" (Just "4-sifret kommunenummer.") QText True Nothing [("placeholder", String "F.eks. 0301"), ("gridXs", Number 6)]
          , field "t5_kommunensNavn" "Kommunens navn" Nothing QText True Nothing [("placeholder", String "F.eks. Oslo"), ("gridXs", Number 6)]
          , field "t5_navnSkjemaansvarlig" "Navn skjemaansvarlig" (Just "Fullt navn på kontaktperson for rapporteringen.") QText True Nothing [("gridXs", Number 12)]
          , field "t5_telefonnummer" "Telefonnummer" Nothing QText True Nothing [("placeholder", String "8 siffer"), ("gridXs", Number 6)]
          , field "t5_epostSkjemaansvarlig" "E-post skjemaansvarlig" (Just "Offisiell e-postadresse i kommunen.") QText True Nothing [("placeholder", String "navn@kommune.no"), ("gridXs", Number 6)]
          ]
      ]
  ...
  , bolk "bolk_c_ekostra" "C11-C2. Om eKOSTRA"
      ...
      Nothing
      [ questionsOnly (eKostra "t5_c_eByggesak" "Har kommunen tatt i bruk eByggesak for rapportering av omfang og saksbehandlingstid for byggesøknader, dispensasjonssøknader og søknader om opprettelse/endring av grunneiendom fra 1.1.2024?") ]
  ...
  , bolk "bolk_d_ekostra" "D1-D2. Om eKOSTRA" Nothing Nothing
      [ questionsOnly (eKostra "t5_d_eByggesak" "Har kommunen tatt i bruk eByggesak for rapportering om resultat av saksbehandling for byggesøknader og andre søknader fra 1.1.2024?") ]
  , bolk "bolk_e0" "E. Klagesaksbehandling: Omfang, resultat og saksbehandlingstid" Nothing Nothing
      [ questionsOnly
          [ field "t5_e0a_klagerMottattEllerBehandlet" "E0a. Har kommunen mottatt eller behandlet klager i rapporteringsåret?" Nothing QBoolean True Nothing []
          , field "t5_e0b_statsforvalterBehandlet" "E0b. Har Statsforvalteren behandlet kommunale vedtak angående klager fra kommunen i rapporteringsåret?" Nothing QBoolean True Nothing []
          ]
      ]
  , bolk "bolk_e1" "E1. Antall klagesaker behandlet i kommunen" Nothing (Just (IsTrue "t5_e0a_klagerMottattEllerBehandlet")) [ matrix e1 ]
  , bolk "bolk_e2" "E2. Klagesaker oversendt fra kommunen og behandlet av Statsforvalteren" Nothing (Just (IsTrue "t5_e0b_statsforvalterBehandlet")) [ matrix e2 ]
  , bolk "bolk_f0" "F. Utøvelse av tilsyn ved tiltak (i byggesaker)" Nothing Nothing
      [ questionsOnly [ field "t5_f0a_erUtfoertTilsyn" "F0a. Er det utført tilsyn med tiltak i rapporteringsåret?" Nothing QBoolean True Nothing [] ] ]
  ...
  , bolk "bolk_g0" "G. Pålegg, sanksjoner og andre virkemidler etter tilsyn og ulovlighetsoppfølging" Nothing Nothing
      [ questionsOnly [ field "t5_g0_erGittPaaleggEllerSanksjoner" "G0. Er det gitt pålegg, brukt sanksjoner eller andre virkemidler i rapporteringsåret?" Nothing QBoolean True Nothing [] ] ]
  ...
  , bolk "bolk_h" "H. Kommentarer og merknader til skjemaet"
      ...
      Nothing
      [ questionsOnly [ field "t5_merknader" "Kommentarer og merknader (maks. 999 tegn)" Nothing QTextArea False Nothing [("maxLength", Number 999), ("gridXs", Number 12)] ] ]
  , bolk "bolk_i" "I. Grunnlag for rapportering og tid for utfylling av skjemaet" Nothing Nothing [ bolkI ]
  ]
  where
    tilsyn  = Just (IsTrue "t5_f0a_erUtfoertTilsyn")
    paalegg = Just (IsTrue "t5_g0_erGittPaaleggEllerSanksjoner")
...
fromC10 c10col =
  [ (("1", "a"), Field (cellId "t5_c10" "1" c10col))
  , (("2", "a"), Field (cellId "t5_c10" "2" c10col))
  ]
...
c12, c13, c2 :: Matrix
c12 = deadlineMatrix "t5_c12" "Ett-trinnssøknader med ansvarsrett" "c"
c13 = deadlineMatrix "t5_c13" "Ett-trinnssøknader uten ansvarsrett" "d"
c2  = deadlineMatrix "t5_c2" "Søknader om opprettelse/endring av eiendommer" "f"
...
c14 = Matrix
  ...
  where
    c14Shape = Matrix "t5_c14" (take 4 (caseRows "søknader" [])) (deadlineCols "Byggesøknader") [] []
...
f4 :: FormParts
f4 =
  ( [ field "t5_f4_1" "F4.1 Antall tilsyn der alt var ok. Ingen feil/mangler påvist" Nothing QInteger False Nothing [("gridXs", Number 6)]
    , field "t5_f4_2" "F4.2 Antall tilsyn som avdekket ulovlighet/forhold som krever oppfølging" Nothing QInteger False Nothing [("gridXs", Number 6)]
    ]
  , []
  , [ Constraint
        { constraintId        = "t5_f4_konklusjon"
        , constraintLeft      = sumOf ["t5_f4_1", "t5_f4_2"]
        , comparison          = CmpLte
        , constraintRight     = Field (cellId "t5_f2" "a" "a")
...
bolkI :: FormParts
bolkI =
  ( [ field "t5_elektroniskSakssystemBrukt" "I.1 Er elektronisk sakssystem brukt som grunnlag for store deler av rapporteringen?" Nothing QBoolean True Nothing [("gridXs", Number 6)]
    , field "t5_maskinelleOpptellinger" "I.1a Er tallene i utfyllingen av skjema framkommet som resultat av maskinelle opptellinger/summeringer?" Nothing QBoolean True (Just (IsTrue "t5_elektroniskSakssystemBrukt")) [("gridXs", Number 6)]
    , field "t5_timerTotalt" "I.2 Antall timer det tok å rapportere - totalt (beregnet)" Nothing QInteger False Nothing [("unit", String "Timer"), ("readOnly", Bool True), ("gridXs", Number 4)]
    , field "t5_timerUtfylling" "I.2a Antall timer det tok å fylle ut skjemaet" Nothing QInteger True Nothing [("unit", String "Timer"), ("gridXs", Number 4)]
    , field "t5_timerFremskaffe" "I.2b Antall timer det tok å framskaffe informasjonen for utfylling" Nothing QInteger True Nothing [("unit", String "Timer"), ("gridXs", Number 4)]
    ]
  , [ Calculation "t5_timerTotalt" (sumOf ["t5_timerUtfylling", "t5_timerFremskaffe"]) ]
  , []
  )
